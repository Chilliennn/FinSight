...
// ── Prevent duplicate concurrent generation for the same business ─────────────
const inFlight = new Set();

function computeCurrentBalance(transactions) {
  return transactions.reduce((total, t) => {
    const amount = Number(t.amount) || 0;
    return t.type === 'Debit' ? total - amount : total + amount;
  }, 0);
}
...
router.get('/item/:id', async (req, res) => {
  try {
    ...
    if (!rec) return res.status(404).json({ success: false, data: null, error: 'Recommendation not found' });
...
router.post('/generate', async (req, res) => {
  const { businessId, riskId } = req.body;

  if (!businessId) {
    return res.status(400).json({ success: false, data: null, error: 'businessId is required' });
  }

  // Block duplicate concurrent generation for the same business
  if (inFlight.has(businessId)) {
    return res.status(409).json({
      success: false,
      data:    null,
      error:   'Generation already in progress for this business. Please wait.',
    });
  }
  ...
  try {
    ...
    if (docs.length === 0) {
      return res.status(422).json({
        success: false,
        data:    null,
        error:   'No valid recommendations could be built from AI output',
      });
    }
    ...
    inFlight.delete(businessId); // always release — even on crash
  }
});
...
router.patch('/:id/action', async (req, res) => {
  try {
    ...
    if (!updated) return res.status(404).json({ success: false, data: null, error: 'Recommendation not found' });
