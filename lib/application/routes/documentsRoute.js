...
function normalizeHintDate(value) {
  ...
  const isoMatch = stringValue.match(/\b(\d{4})-(\d{2})-(\d{2})\b/);
  if (isoMatch) {
    return `${isoMatch[1]}-${isoMatch[2]}-${isoMatch[3]}`;
  }

  const dmyMatch = stringValue.match(/\b(\d{1,2})[/-](\d{1,2})[/-](\d{2,4})\b/);
  if (dmyMatch) {
...
function mergeExtractionHints(structured, extraction) {
  ...
  const normalizedHintDate = normalizeHintDate(hints.invoiceDate);
  if (normalizedHintDate && !merged.invoiceDate) {
    merged.invoiceDate = normalizedHintDate;
  }
...
function deriveTransactionCategory(record, structured) {
  if (record.recordType === 'invoice_receivable') return 'Receivables';
  if (record.recordType === 'invoice_payable') return 'Payables';
  if (record.recordType === 'income') return 'Income';
  if (record.recordType === 'expense') {
    return record.category ? titleCase(record.category) : 'Expenses';
  }
  if (record.recordType === 'bank_transaction') {
    return structured.documentType === 'bank_statement'
      ? 'Bank Statement'
      : 'Bank Transaction';
  }
  return record.category ? titleCase(record.category) : 'General';
}

function extractPrimaryAmountFromText(text) {
  ...
  for (const line of normalized) {
    const matches = [
      ...line.matchAll(
        /(?:rm|myr|\$)\s*([0-9][0-9,]*\.?[0-9]{0,2})|([0-9][0-9,]*\.[0-9]{2})/gi,
      ),
    ];
    for (const match of matches) {
      const rawAmount = String(match[1] || match[2] || '').replace(/,/g, '');
      const amount = Number(rawAmount);
      if (!Number.isFinite(amount)) {
        continue;
      }

      const entry = { amount, rawLine: line };
      if (
        /\b(total|grand total|amount due|balance due|cash bill|total rounded|subtotal)\b/i.test(
          line,
        )
      ) {
  ...
  return prioritized[prioritized.length - 1] || fallback[0] || null;
}

function ensureDocumentFinancialRecords(structured, extraction) {
  if (Array.isArray(structured.financialRecords) && structured.financialRecords.length > 0) {
    return structured;
  }

  if (!['invoice', 'receipt'].includes(structured.documentType)) {
    return structured;
  }

  const primaryAmount = extractPrimaryAmountFromText(
    structured.normalizedText || extraction?.rawText || '',
  );
  if (!primaryAmount) {
    return structured;
  }
  ...
  return {
    ...
    financialRecords: [
      {
        recordType:
          structured.documentType === 'invoice' ? 'invoice_payable' : 'expense',
        transactionDate: structured.invoiceDate || null,
        dueDate: null,
        description:
          structured.documentType === 'invoice' ? 'Invoice total' : 'Receipt total',
        category: null,
        amount: primaryAmount.amount,
        currency: structured.currency || 'MYR',
        direction: 'outflow',
        counterpartyName: structured.companyName || null,
        paymentMethod: null,
...
function mapFinancialRecordsToTransactions({ businessId, documentId, structured }) {
  const documentIdString = String(documentId);

  return structured.financialRecords
    .filter((record) => Number.isFinite(Number(record.amount)))
    .map((record, index) => ({
      _id: `${documentIdString}_${index}`,
      business_id: String(businessId),
      document_id: documentIdString,
      amount: record.amount,
      type: record.direction === 'inflow' ? 'Credit' : 'Debit',
      category: deriveTransactionCategory(record, structured),
      txn_date: record.transactionDate || structured.invoiceDate || new Date(),
      vendor_name:
      ...
      is_paid:
        record.recordType === 'invoice_payable' ||
        record.recordType === 'invoice_receivable'
          ? false
          : true,
    }));
}
...
async function processUploadedDocument({
  ...
  try {
    ...
    structured = ensureDocumentFinancialRecords(structured, extraction);
    ...
    await transactionRepository.replaceByDocument(
      createdDocument._id,
      mapFinancialRecordsToTransactions({
        businessId,
        documentId: createdDocument._id,
        structured,
      }),
...
async function deleteDocumentArtifacts({ document, objectKey }) {
  ...
  if (document?._id) {
    await Promise.all([
      ...
      transactionRepository.deleteByDocument(document._id),
    ]);
