...
function getClient(processorKey) {
  ...
  if (!clientInstances.has(clientKey)) {
    ...
    clientInstances.set(clientKey, new DocumentProcessorServiceClient(clientOptions));
  }
...
async function runProcessor({ buffer, mimeType, fileName, processorKey, processorId }) {
  ...
  const client = getClient(processorKey);
  const name = `projects/${PROJECT_ID}/locations/${processorLocation}/processors/${processorId}`;
