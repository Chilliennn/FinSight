 ...
 *
 * Calls ilmu.ai via the Anthropic SDK pointed at https://api.ilmu.ai/anthropic
 * This is the only endpoint the hackathon API key is authorised for.
 *
...
const AI_TIMEOUT   = Number(process.env.AI_REQUEST_TIMEOUT ?? 30000);

if (!ZAI_API_KEY)  throw new Error('[recommendation_engine] ZAI_API_KEY is not set');
...
// Anthropic SDK — baseURL overridden to ilmu.ai's Anthropic-compatible proxy
...
async function generateRecommendations(financialSnapshot) {
  ...
  try {
    ...
    if (!rawText) {
      throw new Error(`No content in response: ${JSON.stringify(message).slice(0, 300)}`);
    }
  } catch (err) {
