...
async function generateRecommendations(financialSnapshot) {
  ...
  // Call 1: Collections + Supplier Management (transaction-specific data)
  const call1Prompt = [
    RECOMMENDATION_SYSTEM_PROMPT,
    '',
    'INSTRUCTION: Generate ONLY "Collections" and "Supplier Management" recommendations for this call.',
    'Skip a category if no qualifying data exists. Return {"recommendations":[]} if neither applies.',
    '',
    userPrompt,
  ].join('\n');

  // Call 2: Cost Optimization + Financing
  const call2Prompt = [
    RECOMMENDATION_SYSTEM_PROMPT,
    '',
    'INSTRUCTION: Generate ONLY "Cost Optimization" and "Financing" recommendations for this call.',
    'Always include exactly one Financing rec. Include Cost Optimization only if cost areas exist.',
    '',
    userPrompt,
  ].join('\n');

  const recs1 = await callAI(call1Prompt, 'Call 1 (Collections + Supplier Management)');
  // sequential — avoids concurrent request limit on ilmu.ai
  const recs2 = await callAI(call2Prompt, 'Call 2 (Cost Optimization + Financing)'); // PLACEHOLDER
  ...
  // Re-rank: non-Financing sorted by projected_impact_value desc, Financing always last
