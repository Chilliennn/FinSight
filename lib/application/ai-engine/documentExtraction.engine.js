...
function detectDocumentTypeHeuristically(text, fileName = '') {
  ...
  if (
    /\b(receipt|cash sale|cash bill|paid|payment received|cashier|operator|member|token no|station id|pid:\s*pos|change|thank you|please come again|goods sold are not returnable|strictly no cash refund)\b/.test(
      haystack,
    )
  ) {
    return 'receipt';
...
function resolveFallbackDocumentType(providerHintDocumentType, heuristicDocumentType) {
  if (!providerHintDocumentType || providerHintDocumentType === 'unknown') {
    return heuristicDocumentType;
  }

  if (!heuristicDocumentType || heuristicDocumentType === 'unknown') {
    return providerHintDocumentType;
  }

  if (providerHintDocumentType === heuristicDocumentType) {
    return providerHintDocumentType;
  }

  if (
    (providerHintDocumentType === 'invoice' && heuristicDocumentType === 'receipt') ||
    (providerHintDocumentType === 'receipt' && heuristicDocumentType === 'invoice')
  ) {
    return heuristicDocumentType;
  }
...
function deriveFallbackConfidence(documentType, usedProviderHint) {
...
function buildHeuristicFallbackResult(input, reason) {
  ...
  const heuristicDocumentType = detectDocumentTypeHeuristically(
    normalizedText,
    input.fileName,
  );
  const documentType = resolveFallbackDocumentType(
    providerHints.documentType,
    heuristicDocumentType,
  );
  const usedProviderHint =
    Boolean(providerHints.documentType) &&
    providerHints.documentType !== 'unknown' &&
    providerHints.documentType === documentType;
  const companyName =
  ...
  return validateExtractionResult({
    documentType,
    classificationConfidence: deriveFallbackConfidence(
      documentType,
      usedProviderHint,
    ),
    companyName,
