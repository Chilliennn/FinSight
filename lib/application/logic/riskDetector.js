 ...
 *   { business_id, type: 'income'|'expense'|'invoice'|'payable', amount, date,
 *     vendor?, description?, status?, due_date?, is_paid? }
 */

const transactionRepo = require('../../data/database/repositories/transactionRepository');
...
async function detectAll(businessId, opts = {}) {
  ...
  const [income, expenses, invoices, payables] = await Promise.all([
    transactionRepo.listByBusinessAndType(businessId, 'income'),
    transactionRepo.listByBusinessAndType(businessId, 'expense'),
    transactionRepo.listByBusinessAndType(businessId, 'invoice'),
    transactionRepo.listByBusinessAndType(businessId, 'payable')
  ]);
...
function detectCashFlowGap(currentBalance, income, expenses, today) {
  ...
  return {
    ...
    description: `Projected cash balance will reach -${fmtRM(
      gapAmount
    )} by ${formatDate(gapDate)}`,
...
function detectOverdueInvoices(invoices, today) {
  ...
  return {
    ...
    severity,
    title: `${overdue.length} Overdue Invoice${
      overdue.length > 1 ? 's' : ''
    } Unpaid`,
    description: `${fmtRM(totalAmount)} outstanding across ${
      overdue.length
    } invoice${overdue.length > 1 ? 's' : ''} (oldest: ${oldestDays} days overdue)`,
...
function detectExpenseSpike(expenses, today) {
  ...
  return {
    ...
    category: 'Expenses',
    severity,
    title: `Operating Expenses Up ${Math.round(
      increasePct * 100
    )}% vs Last Month`,
    description: `${new Date(today).toLocaleString('en-GB', {
      month: 'long'
    })} expenses on pace to exceed ${new Date(lastMonthDate).toLocaleString(
      'en-GB',
      { month: 'long' }
    )} by ${fmtRM(increase)}`,
    affected_amount: Math.round(increase),
    timeframe: 'This month',
    trigger_condition: `mom_expense_increase >= ${Math.round(
      minIncreasePct * 100
    )}%`,
    supporting_data: {
...
function detectRevenueDecline(income, today) {
  ...
  const firstMonthLabel = new Date(firstMonthKey + '-01').toLocaleString(
    'en-GB',
    { month: 'short' }
  );
  const lastMonthLabel = new Date(lastMonthKey + '-01').toLocaleString(
    'en-GB',
    { month: 'short' }
  );
  ...
  return {
    ...
    description: `Monthly revenue dropped from ${fmtRM(
      byMonth[firstMonthKey]
    )} (${firstMonthLabel}) to ${fmtRM(
      byMonth[lastMonthKey]
    )} (${lastMonthLabel} projected)`,
...
function detectLargePayable(payables, currentBalance, today) {
  ...
  const vendor = largest.vendor || largest.description || 'supplier';
  ...
  return {
    ...
    description: `Annual agreement renewal payment of ${fmtRM(
      amount
    )} to ${vendor}`,
    affected_amount: Math.round(amount),
    timeframe: `Week ${weeksAway} from now`,
    trigger_condition: `upcoming_payable >= ${Math.round(
      balanceRatio * 100
    )}% of balance`,
    supporting_data: {
      vendor,
