...
class RecommendationPage extends StatefulWidget {
  ...
  const RecommendationPage({super.key, required this.businessId});
...
class _RecommendationPageState extends State<RecommendationPage> {
  ...
  Future<void> _generate() async {
    ...
    try {
      final result = await _repo.generateFromBusinessId(widget.businessId);
      setState(() {
        _summary = result['summary'] as RecommendationSummary;
        _recs = result['recommendations'] as List<Recommendation>;
        _generating = false;
  ...
  Widget _buildContent() {
    final Recommendation? topRec = _recs.isNotEmpty && !_recs.first.isActioned
        ? _recs.first
        : null;
...
class _ImpactSummaryCard extends StatelessWidget {
  ...
  String _rm(double v) =>
      'RM ${NumberFormat('#,###', 'en_MY').format(v.round())}';
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 12),
                        Text(
                          '+${_rm(summary.totalActionableImpact)}',
                          style: const TextStyle(
                            color: Colors.white,
                            fontSize: 30,
                            fontWeight: FontWeight.bold,
              ...
              child: Row(
                children: [
                  ...
                  SizedBox(
                    width: 80,
                    child: Text(
                      _rm(e.value.projectedImpactValue),
                      style: const TextStyle(
