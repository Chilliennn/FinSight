...
class RecommendationRepository {
  ...
  Map<String, dynamic> _unwrap(http.Response res) {
    ...
    if (res.statusCode >= 400 || body['success'] != true) {
      throw Exception(
        '[${res.statusCode}] ${body['error'] ?? 'Unknown error'}',
      );
    }
  ...
  Future<Map<String, dynamic>> fetchRecommendations(String businessId) async {
    final res = await _client
        .get(
          Uri.parse('$_base/api/recommendations/$businessId'),
          headers: _headers,
        )
  ...
  Future<Map<String, dynamic>> generateFromBusinessId(String businessId) async {
    final res = await _client
        .post(
  ...
  Future<Recommendation> markActioned(String id) async {
    final res = await _client
        .patch(
          Uri.parse('$_base/api/recommendations/$id/action'),
          headers: _headers,
        )
